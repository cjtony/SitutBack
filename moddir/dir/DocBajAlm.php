...
if ($_SESSION['keyDir'] == "" || $_SESSION['keyDir'] == null) {
	...
	$valAlm = $_GET['v'];
	$valBaj = $_GET['p'];
	$datDir = $director -> userDirDet($keyDir);
	$id_car = $datDir -> id_carrera;
	$datBajVal = $director -> valDatBaj($id_car, base64_decode($valAlm));
	if ($datBajVal -> CantVal == 1) {
		$datAlm = $director -> datAlmAll(base64_decode($valAlm));
		$datEnc = $director -> datEncBaj(base64_decode($valAlm));
		$datBaj = $director -> datBajAlm(base64_decode($valBaj));
		$datBec = $director -> datBecAlm(base64_decode($valAlm));
		//PDF
		...
		$pdf -> SetFont('Arial', 'B', 8);
		$pdf -> SetX(5);
		$pdf -> Cell(40, 8, utf8_decode('MOTIVO DE LA BAJA'), 0, 0, 'L');
		$pdf -> SetFont('Arial', '', 8);
